...
export default function CollectionSubscribeComponent({ route }: any) {
  const navigation = useNavigation();
  const [code, setCode] = useState('');
  const [error, setError] = useState(false);

  const { courseId = null, contain, token } = route.params || {};
  const [subscribed, setSubscribed] = useState(contain);

  const subscribeCourse = () => {
    if (code.length === 6) {
      subscribe({ collection_code: code, course_code: null, item_id: courseId }, token)
        .then((response: any) => {
          setSubscribed(true);
          navigation.goBack();
        })
